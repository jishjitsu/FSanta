...
then the user-word is returned with the matached word in the dictionary.
...
public class day32_2 {
    ...
    public static void main(String[] args) {
        ...
        List<String> result = wordCorrector(dict, userWords);
        System.out.println(result);
    }

    static Set<Character> vowels = new HashSet<>(Arrays.asList('a', 'e', 'i', 'o', 'u'));

    public static List<String> wordCorrector(String[] dict, String[] userWords) {
        ...
        for (String word : dict) {
            ...
            vowelErrorMap.putIfAbsent(vowelNormalized, word);
        }

        List<String> result = new ArrayList<>();
        for (String userWord : userWords) {
            if (exactWords.contains(userWord)) {
                result.add(userWord);
                continue;
            }

            String lowerUser = userWord.toLowerCase();
            if (caseInsensitiveMap.containsKey(lowerUser)) {
                result.add(caseInsensitiveMap.get(lowerUser));
                continue;
            }

            String normalized = normalizeVowels(lowerUser);
            if (vowelErrorMap.containsKey(normalized)) {
                result.add(vowelErrorMap.get(normalized));
                continue;
            }
    ...
    private static String normalizeVowels(String word) {
