...
public class day35_1{
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 26; i++) {
            minOps = Math.min(minOps, len1 - freq1[i] + len2 - freq2[i]);
        }

        for (int i = 1; i < 26; i++) {
            int ops = 0, sum1 = 0, sum2 = 0;
            for (int j = 0; j < i; j++) sum1 += freq1[j];
            for (int j = i; j < 26; j++) sum2 += freq2[j];
            ops = (len1 - sum1) + (len2 - sum2);
            minOps = Math.min(minOps, ops);
        }

        for (int i = 1; i < 26; i++) {
            int ops = 0, sum1 = 0, sum2 = 0;
            for (int j = 0; j < i; j++) sum2 += freq2[j];
            for (int j = i; j < 26; j++) sum1 += freq1[j];
            ops = (len2 - sum2) + (len1 - sum1);
            minOps = Math.min(minOps, ops);
        }
