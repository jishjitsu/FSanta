...
public class day16_2{
    public static int startingStation(int[] fuel, int[] cost){
        int n=fuel.length;
        int total_fuel=0, total_cost=0, start=0, current_fuel=0;
        
        for (int i=0; i<n; i++){
            total_fuel+=fuel[i];
            total_cost+=cost[i];
            current_fuel+=fuel[i]-cost[i];
        ...
        return(total_fuel>=total_cost) ? start:-1;
    }
    ...
    public static void main(String[] args){
        ...
        System.out.println(startingStation(fuel, cost));
    }
}
